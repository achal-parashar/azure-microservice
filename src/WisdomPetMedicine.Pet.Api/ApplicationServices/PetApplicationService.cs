...
namespace WisdomPetMedicine.Pet.Api.ApplicationServices
{
    public class PetApplicationService
    {
        ...
        public PetApplicationService(IPetRepository petRepository,
                                     IBreedService breedService,IConfiguration configuration)
        {
            this.petRepository = petRepository;
            this.breedService = breedService;
            DomainEvents.PetFlaggedForAdoption.Register(async c => 
            {
                var integrationEvent = new PetFlaggedForAdoptionIntegration()
                {
                    Id = c.Id,
                    Name = c.Name,
                    Breed = c.Breed,
                ...
                await PublishIntegratinEventAsync(integrationEvent, configuration["ServiceBus:ConnectionString"],
                     configuration["ServiceBus:Adoption:TopicName"]);
            });
            DomainEvents.PetTransferredToHospital.Register(async c =>
            {
                var integrationEvent = new PetTransferredToHospitalIntegration()
                {
                    Id = c.Id,
                    Name = c.Name,
                    Breed = c.Breed,
        ...
        private async Task PublishIntegratinEventAsync(IIntegrationEvent integrationEvent, string connectionString,
           string topicName)
        {
